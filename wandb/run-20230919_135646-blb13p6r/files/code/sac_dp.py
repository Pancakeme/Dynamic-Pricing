...
def parse_args():
    ...
    parser.add_argument("--wandb-entity", type=str, default="ee21b138",
...
class SoftQNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        self.fc1 = nn.Linear(np.array([1]).prod() + np.prod([1]), 256)
...
class Actor(nn.Module):
    def __init__(self):
        super().__init__()
        self.fc1 = nn.Linear(np.array([1]).prod(), 256)
        self.fc2 = nn.Linear(256, 256)
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
        ...
        next_obs = [next_comp_price]
        
        value_self = actions[0] + np.random.uniform(0, 1) * quality_self + np.random.uniform(0, 0.5) * (100 - rating_self)
        value_comp = obs[0] + np.random.uniform(0, 1) * quality_comp + np.random.uniform(0, 0.5) * (100 - rating_comp)
